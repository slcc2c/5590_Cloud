			...
			d3.csv("cp3.csv", function(data) {
			    ...
			    var yScale = d3.scale.linear()
							.domain([0, d3.max(data, function(d){
                                        return filterInactive(d.AST)/filterInactive(d.MP.substr(0,2));
                                    })])
						    .range([0, h]);
			   ...
			   .attr("y", function(d){ return h-yScale(filterInactive(d.AST)/filterInactive(d.MP.substr(0,2)))})
			   .attr("width", xScale.rangeBand())
			   .attr("height", function(d) { return yScale(filterInactive(d.AST)/filterInactive(d.MP.substr(0,2))) })
			   .attr("fill", function(d) {
					return "rgb(200, 0, " + parseInt(colorScale(filterInactive(d.PTS))) + ")";
			   })
               .attr("fill-opacity", function(d){
                   return opacityScale(filterInactive(d.AST)/filterInactive(d.MP.substr(0,2)));
               })
               ...
               .enter()
               .append("text")
               .text(function(d,i){return (filterInactive(d.AST)/filterInactive(d.MP.substr(0,2))).toString().substr(0,5)})
               .attr("text-anchor", "middle")
               .attr("x", function(d, i) {
                   if (i<=data.length-2){
			   		return (xScale(i)+xScale(i+1))/2;
